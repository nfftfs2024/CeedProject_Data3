...
def nested_list_tag(elem, space, type, answer):
    ...
    for i in elem.children:
        ...
        if i.name == 'ul':
            answer = nested_list_tag(i, " " * 2, 'ul', answer)
        # For ol children tags, recursive nested extraction
        if i.name == 'ol':
            answer = nested_list_tag(i, " " * 2, 'ol', answer)
    return answer
...
def html_main(ques, ans, ad):
    ...
    #### Soup on local files
    # dir_path = r"C:\Users\jchou\Desktop\Code\html"
    # t = ['Sit-Stand Work stations','Office Ergonomics for Mobile Workers', 'Office Ergonomics for Tall Workers','HSE Services',]
